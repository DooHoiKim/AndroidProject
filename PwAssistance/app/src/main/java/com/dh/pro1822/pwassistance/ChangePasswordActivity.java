...
public class ChangePasswordActivity extends Activity implements View.OnClickListener {
    ...
    private static final int INPUT_MODE_OLD = 0;
    private static final int INPUT_MODE_NEW = 1;
    private static final int INPUT_MODE_NEW_AGAIN = 2;
            ...
            switch (INPUT_MODE) {
                case INPUT_MODE_OLD:
                    if (inputPassword.equals(getLoginPw())) {
                        ...
                        toast.show();
                        */
                        inputInfo.setText(R.string.pw_new);
                        INPUT_MODE = INPUT_MODE_NEW;
                    } else {
                        inputInfo.setText(R.string.pw_exist_again);
                    }
                    initPassword();
                    break;
                case INPUT_MODE_NEW:
                    inputNewPassword = inputPassword;
                    inputInfo.setText(R.string.pw_new2);
                    INPUT_MODE = INPUT_MODE_NEW_AGAIN;
                    initPassword();
                    break;
                case INPUT_MODE_NEW_AGAIN:
                    if (inputNewPassword.equals(inputPassword)) {
                        // DB Save process
                        ContentValues contentValues = new ContentValues();
                        contentValues.put(ContractDB.LoginEntry.COLUMN_LOG_IN_PW, inputNewPassword);

                        int count = db.update(ContractDB.LoginEntry.TABLE_NAME, contentValues,
                                ContractDB.PwListEntry.COLUMN_ID + " = 1", null);
                        ...
                        if (count == 0) {
                            ...
                            inputInfo.setText(R.string.pw_complete);
                        }

                    } else {
                        inputInfo.setText(R.string.pw_new_again);
                    }
                    initPassword();
                    break;
            }
